...
def test_reads_every_recent_word(db_path):
    importer = KindleImporter(db_path, "pl", do_translate=False,
                              import_days=DAYS_COVERING_ALL_WORDS)
...
def test_skips_words_older_than_the_window(db_path):
    importer = KindleImporter(db_path, "pl", do_translate=False,
                              import_days=DAYS_COVERING_NO_WORDS)
    importer.translate_words_from_db()
...
def test_each_word_is_translated_into_the_target_language(db_path, monkeypatch):
    ...
    importer = KindleImporter(db_path, "de", do_translate=True,
                              import_days=DAYS_COVERING_ALL_WORDS)
    importer.translate_words_from_db()

    for word, translation in zip(importer.words, importer.translated):
...
def test_failed_translation_is_recorded_as_cannot_translate(db_path, monkeypatch):
    ...
    importer = KindleImporter(db_path, "pl", do_translate=True,
                              import_days=DAYS_COVERING_ALL_WORDS)
    importer.translate_words_from_db()
...
def test_include_usage_embeds_the_bolded_example_sentence(db_path):
    importer = KindleImporter(db_path, "pl", include_usage=True, do_translate=False,
                              import_days=DAYS_COVERING_ALL_WORDS)
    importer.translate_words_from_db()
...
def test_create_temporary_file_writes_word_then_translation_per_line(db_path, monkeypatch):
    monkeypatch.setattr(kindleimporter, "translate",
                        lambda word, to_lang=None: f"translated-{word}")
